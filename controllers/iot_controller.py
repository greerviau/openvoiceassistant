...
class IOTController(object):
    ...
    def light_control(self, command, room_id):
        word_list = command.split()
        if 'on' in word_list or 'max' in word_list or 'up' in word_list:
            self.client.publish(f'home/virtual_assistant/{room_id}/lights_state', 1)
        elif 'off' in word_list:
            self.client.publish(f'home/virtual_assistant/{room_id}/lights_state', 0)
        elif 'dim' in word_list or 'down' in word_list:
            self.client.publish(f'home/virtual_assistant/{room_id}/lights_dim', 1)
